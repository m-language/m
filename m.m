...
(def symbol-expr (fn name (fn path (fn start (fn end (fn f (fn _ ((((f name) path) start) end))))))))
(def list-expr (fn exprs (fn path (fn start (fn end (fn _ (fn f ((((f exprs) path) start) end))))))))
...
(def symbol-expr.name (fn expr ((expr (fn name (fn _ (fn _ (fn _ name))))) (fn _ (fn _ (fn _ (fn _ (error (symbol "Could not cast expr to symbol-expr")))))))))
(def list-expr.exprs (fn expr ((expr (fn _ (fn _ (fn _ (fn _ (error (symbol "Could not cast expr to list-expr"))))))) (fn exprs (fn _ (fn _ (fn _ exprs)))))))
...
(def expr.with-path (fix (fn expr.with-path (fn path (fn expr ((expr (fn name (fn _ (fn start (fn end ((((symbol-expr name) path) start) end)))))) (fn exprs (fn _ (fn start (fn end ((((list-expr ((map exprs) (expr.with-path path))) path) start) end)))))))))))
...
(def match-fn-expr' (fn exprs (fn names (fn invalid-arg (fn success ((((nil? (cdr exprs)) (fn _ ((success (reverse names)) (car exprs)))) (fn _ (((car exprs) (fn name (fn _ (fn _ (fn _ ((((match-fn-expr' (cdr exprs)) ((cons name) names)) invalid-arg) success)))))) (fn _ (fn _ (fn _ (fn _ (invalid-arg (car exprs))))))))) nil))))))
...
(def match-def-expr (fn exprs (fn no-name (fn no-expr (fn invalid-name (fn success ((((nil? exprs) (fn _ no-name)) (fn _ ((((nil? (cdr exprs)) (fn _ no-expr)) (fn _ (((car exprs) (fn name (fn _ (fn _ (fn _ ((success name) (cadr exprs))))))) (fn _ (fn _ (fn _ (fn _ (invalid-name (car exprs))))))))) nil))) nil)))))))
...
(def closures' (fix (fn closures' (fn local-env (fn acc (fn expr ((expr (fn name (fn _ (fn _ (fn _ ((((null? ((tree-map.get (local-env.locals local-env)) name)) (fn _ acc)) (fn _ (((tree-map.put acc) name) true))) nil)))))) (fn exprs (fn _ (fn _ (fn _ (((fold exprs) acc) (closures' local-env)))))))))))))

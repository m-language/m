...
(def char->nat id)
(def coproduct/pure (fn value (fn f (f value))))
(def coproduct/left (fn coproduct (fn _ (fn right (coproduct right)))))
(def coproduct/right (fn coproduct (fn left (fn _ (coproduct left)))))
...
(def product/nil (fn f f))
(def product/cons (fn value (fn product (fn f (product (f value))))))
(def product (fn env (fn exprs (result/success ((((nil? exprs) (const (expr/symbol (symbol product/nil)))) (fn _ (expr/list ((cons (expr/symbol (symbol product/cons))) ((cons (car exprs)) ((cons (expr/list ((cons (expr/symbol (symbol list))) (cdr exprs)))) nil)))))) nil)))))
...
(def match-fn-expr' (fn exprs (fn names (fn invalid-arg (fn success ((((nil? (cdr exprs)) (fn _ ((success (reverse names)) (car exprs)))) (fn _ (((expr.match (car exprs)) (fn name (fn _ ((((match-fn-expr' (cdr exprs)) ((cons name) names)) invalid-arg) success)))) (fn _ (fn _ (invalid-arg (car exprs))))))) nil))))))
...
(def match-def-expr (fn exprs (fn no-name (fn no-expr (fn extra-exprs (fn invalid-name (fn success ((((nil? exprs) (fn _ no-name)) (fn _ ((((nil? (cdr exprs)) (fn _ no-expr)) (fn _ ((((not (nil? (cddr exprs))) (fn _ extra-exprs)) (fn _ (((expr.match (car exprs)) (fn name (fn _ ((success name) (cadr exprs))))) (fn _ (fn _ (invalid-name (car exprs))))))) nil))) nil))) nil))))))))
(def match-macro-expr match-def-expr)
(def match-symbol-literal-expr (fn exprs (fn no-symbol (fn extra-exprs (fn invalid-symbol (fn success ((((nil? exprs) (fn _ no-symbol)) (fn _ ((((not (nil? (cdr exprs))) (fn _ extra-exprs)) (fn _ (((expr.match (car exprs)) (fn name (fn _ (success name)))) (fn _ (fn _ (invalid-symbol (car exprs))))))) nil))) nil)))))))
...
(def parse-expr (fix (fn parse-expr (fn path (fn input (fn position (fn continue ((with (car input)) (fn head (((((char.= head) open-parentheses) (fn _ (((((parse-list parse-expr) path) (cdr input)) (next-char position)) (fn exprs (fn path (fn input (fn position' ((((continue ((list-expr exprs) ((location path) ((span position) position')))) path) input) position')))))))) (fn _ (((((char.= head) quote) (fn _ ((((parse-symbol-literal path) (cdr input)) (next-char position)) (fn chars (fn path (fn input (fn position' ((((continue ((symbol-expr chars) ((location path) ((span position) position')))) path) input) position')))))))) (fn _ ((((parse-symbol path) input) position) (fn chars (fn path (fn input (fn position' ((((continue ((symbol-expr chars) ((location path) ((span position) position')))) path) input) position')))))))) nil))) nil))))))))))
...
(def closures' (fix (fn closures' (fn local-env (fn acc (fn expr (((expr.match expr) (fn name (fn _ ((((null? ((tree-map.get (local-env.locals local-env)) name)) (fn _ acc)) (fn _ (((tree-map.put acc) name) true))) nil)))) (fn exprs (fn _ (((fold exprs) acc) (closures' local-env)))))))))))

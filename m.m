...
(def defrec (fn env (fn exprs (result/success (expr/list ((cons (expr/symbol (symbol def))) ((cons (car exprs)) ((cons (expr/list ((cons (expr/symbol (symbol fix))) ((cons (cadr exprs)) ())))) ()))))))))
(def defnrec (fn env (fn exprs (result/success (expr/list ((cons (expr/symbol (symbol defrec))) ((cons (car exprs)) ((cons (expr/list ((cons (expr/symbol (symbol fn))) exprs))) ()))))))))
...
(def init (fix (fn init (fn list ((((nil? (cdr list)) (fn _ ())) (fn _ ((cons (car list)) (init (cdr list))))) ())))))
(def last (fix (fn last (fn list ((((nil? (cdr list)) (fn _ (car list))) (fn _ (last (cdr list)))) ())))))
(def append (fix (fn append (fn list (fn elem ((((nil? list) (fn _ ((cons elem) ()))) (fn _ ((cons (car list)) ((append (cdr list)) elem)))) ()))))))
(def concat (fix (fn concat (fn a (fn b ((((nil? a) (fn _ b)) (fn _ ((cons (car a)) ((concat (cdr a)) b)))) ()))))))
(def get (fix (fn get (fn list (fn n ((((nat.0? n) (fn _ (car list))) (fn _ ((get (cdr list)) (nat.dec n)))) ()))))))
(def map (fix (fn map (fn list (fn f ((((nil? list) (fn _ ())) (fn _ ((cons (f (car list))) ((map (cdr list)) f)))) ()))))))
...
(def flat-map (fix (fn flat-map (fn list (fn f ((((nil? list) (fn _ ())) (fn _ ((concat (f (car list))) ((flat-map (cdr list)) f)))) ()))))))
(def filter (fix (fn filter (fn list (fn f ((((nil? list) (fn _ ())) (fn _ ((((f (car list)) (fn _ ((cons (car list)) ((filter (cdr list)) f)))) (fn _ ((filter (cdr list)) f))) ()))) ()))))))
(def fold (fix (fn fold (fn list (fn acc (fn f ((((nil? list) (fn _ acc)) (fn _ (((fold (cdr list)) ((f acc) (car list))) f))) ())))))))
...
(def reverse' (fix (fn reverse' (fn list (fn acc ((((nil? list) (fn _ acc)) (fn _ ((reverse' (cdr list)) ((cons (car list)) acc)))) ()))))))
(def reverse (fn list ((reverse' list) ())))
(def nat.+ (fix (fn nat.+ (fn a (fn b ((((nat.0? b) (fn _ a)) (fn _ ((nat.+ (nat.inc a)) (nat.dec b)))) ()))))))
...
(def nat.- (fix (fn nat.- (fn a (fn b ((((nat.0? b) (fn _ a)) (fn _ ((nat.- (nat.dec a)) (nat.dec b)))) ()))))))
(def char->nat id)
...
(def list.= (fix (fn list.= (fn = (fn a (fn b ((((nil? a) (fn _ (nil? b))) (fn _ ((((nil? b) (fn _ false)) (fn _ ((and ((= (car a)) (car b))) (fn _ (((list.= =) (cdr a)) (cdr b)))))) ()))) ())))))))
(def nat.< (fix (fn nat.< (fn a (fn b ((((nat.0? a) (fn _ (not (nat.0? b)))) (fn _ ((((nat.0? b) (fn _ false)) (fn _ ((nat.< (nat.dec a)) (nat.dec b)))) ()))) ()))))))
(def nat.> (swap nat.<))
(def nat.= (fix (fn nat.= (fn a (fn b ((((nat.0? a) (fn _ (nat.0? b))) (fn _ ((((nat.0? b) (fn _ false)) (fn _ ((nat.= (nat.dec a)) (nat.dec b)))) ()))) ()))))))
(def char.= nat.=)
(def newline? (fn char ((or ((char.= char) linefeed)) (fn _ ((char.= char) carriage-return)))))
(def parse-comment (fix (fn parse-comment (fn path (fn input (fn position (fn continue (((((or (nil? input)) (fn _ (newline? (car input)))) (fn _ (((continue path) input) position))) (fn _ ((((parse-comment path) (cdr input)) (next-char position)) continue))) ()))))))))
...
(def new-data' (fix (fn new-data' (fn type (fn names (fn fields ((((nil? names) (fn _ ((data type) fields))) (fn _ (fn value (((new-data' type) (cdr names)) ((cons ((pair (car names)) value)) fields))))) ())))))))
...
(def parse-symbol (fix (fn parse-symbol (fn path (fn input (fn position (fn continue ((with (car input)) (fn head (((((or (nil? input)) (fn _ (separator? head))) (fn _ ((((continue ()) path) input) position))) (fn _ ((((parse-symbol path) (cdr input)) (next-char position)) (fn chars (fn path (fn input (fn position ((((continue ((cons head) chars)) path) input) position)))))))) ()))))))))))
...
(def desugar-should-quote? (fix (fn desugar-should-quote? (fn name ((((nil? name) (fn _ false)) (fn _ ((with (car name)) (fn char ((or ((char.= char) quote)) (fn _ ((or ((char.= char) backslash)) (fn _ ((or ((char.= char) open-parentheses)) (fn _ ((or ((char.= char) close-parentheses)) (fn _ ((or ((char.= char) semicolon)) (fn _ ((or (whitespace? char)) (fn _ (desugar-should-quote? (cdr name)))))))))))))))))) ())))))
...
(def compare-list (fix (fn compare-list (fn compare (fn a (fn b (((((and (nil? a)) (fn _ (nil? b))) (fn _ compare=)) (fn _ ((((nil? a) (fn _ compare<)) (fn _ ((((nil? b) (fn _ compare>)) (fn _ ((with ((compare (car a)) (car b))) (fn compare-result ((((compare=? compare-result) (fn _ (((compare-list compare) (cdr a)) (cdr b)))) (fn _ compare-result)) ()))))) ()))) ()))) ())))))))
...
(def parse-single-quote (fix (fn parse-single-quote (fn path (fn input (fn position (fn continue ((((nil? input) (fn _ (error (symbol ""Unexpected end of file"")))) (fn _ ((with (car input)) (fn head (((((char.= head) quote) (fn _ ((((continue ()) path) (cdr input)) (next-char position)))) (fn _ ((((parse-single-quote path) (cdr input)) ((((((char.= head) linefeed) (fn _ next-line)) (fn _ next-char)) ()) position)) (fn chars (fn path (fn input (fn position ((((continue ((cons head) chars)) path) input) position)))))))) ()))))) ()))))))))
(def parse-double-quote (fix (fn parse-double-quote (fn path (fn input (fn position (fn continue ((((nil? input) (fn _ (error (symbol ""Unexpected end of file"")))) (fn _ ((with (car input)) (fn head (((((and ((char.= head) quote)) (fn _ ((char.= (cadr input)) quote))) (fn _ ((((continue ()) path) (cddr input)) (next-char (next-char position))))) (fn _ ((((parse-double-quote path) (cdr input)) ((((((char.= head) linefeed) (fn _ next-line)) (fn _ next-char)) ()) position)) (fn chars (fn path (fn input (fn position ((((continue ((cons head) chars)) path) input) position)))))))) ()))))) ()))))))))
(def parse-symbol-literal (fix (fn parse-symbol-literal (fn path (fn input (fn position (fn continue ((((nil? input) (fn _ (error (symbol ""Unexpected end of file"")))) (fn _ (((((char.= (car input)) quote) (fn _ ((((parse-double-quote path) (cdr input)) (next-char position)) continue))) (fn _ ((((parse-single-quote path) input) position) continue))) ()))) ()))))))))
...
(def repl-parse (fn line (fn index ((((parse-expr ((mangle-fn-name (symbol repl)) index)) ((append line) linefeed)) ((position index) nat.1)) (fn expr (fn path (fn input (fn position expr))))))))
(def parse (fix (fn parse (fn path (fn input (fn position ((((parse-unused path) input) position) (fn path (fn input (fn position ((((nil? input) (fn _ ())) (fn _ ((((parse-expr path) input) position) (fn expr (fn path (fn input (fn position ((cons expr) (((parse path) input) position))))))))) ())))))))))))
...
(def tree-map-node.min-node (fix (fn tree-map-node.min-node (fn node ((((tree-map-node-nil? (tree-map-node.left node)) (fn _ node)) (fn _ (tree-map-node.min-node (tree-map-node.left node)))) ())))))
...
(def tree-map-node.get (fix (fn tree-map-node.get (fn node (fn compare (fn key ((((tree-map-node-nil? node) (fn _ null)) (fn _ ((((fold-compare ((compare key) (tree-map-node.key node))) (fn < (((tree-map-node.get (tree-map-node.left node)) compare) key))) (fn > (((tree-map-node.get (tree-map-node.right node)) compare) key))) (fn = (some (tree-map-node.value node)))))) ())))))))
(def tree-map-node.remove (fix (fn tree-map-node.remove (fn node (fn compare (fn key ((((tree-map-node-nil? node) (fn _ node)) (fn _ ((((fold-compare ((compare key) (tree-map-node.key node))) (fn < ((((tree-map-node (((tree-map-node.remove (tree-map-node.left node)) compare) key)) (tree-map-node.right node)) (tree-map-node.key node)) (tree-map-node.value node)))) (fn > ((((tree-map-node (tree-map-node.left node)) (((tree-map-node.remove (tree-map-node.right node)) compare) key)) (tree-map-node.key node)) (tree-map-node.value node)))) (fn = ((((tree-map-node-nil? (tree-map-node.left node)) (fn _ (tree-map-node.right node))) (fn _ ((((tree-map-node-nil? (tree-map-node.right node)) (fn _ (tree-map-node.left node))) (fn _ ((with (tree-map-node.min-node (tree-map-node.right node))) (fn min ((((tree-map-node (tree-map-node.left node)) (tree-map-node.right (((tree-map-node.remove (tree-map-node.right node)) compare) key))) (tree-map-node.key min)) (tree-map-node.value min)))))) ()))) ()))))) ())))))))
(def tree-map-node.put (fix (fn tree-map-node.put (fn node (fn compare (fn key (fn value ((((tree-map-node-nil? node) (fn _ ((((tree-map-node tree-map-node-nil) tree-map-node-nil) key) value))) (fn _ ((((fold-compare ((compare key) (tree-map-node.key node))) (fn < ((((tree-map-node ((((tree-map-node.put (tree-map-node.left node)) compare) key) value)) (tree-map-node.right node)) (tree-map-node.key node)) (tree-map-node.value node)))) (fn > ((((tree-map-node (tree-map-node.left node)) ((((tree-map-node.put (tree-map-node.right node)) compare) key) value)) (tree-map-node.key node)) (tree-map-node.value node)))) (fn = ((((tree-map-node (tree-map-node.left node)) (tree-map-node.right node)) key) value))))) ()))))))))
(def tree-map-node.fold (fix (fn tree-map-node.fold (fn node (fn acc (fn f ((((tree-map-node-nil? node) (fn _ acc)) (fn _ (((tree-map-node.fold (tree-map-node.right node)) (((f (((tree-map-node.fold (tree-map-node.left node)) acc) f)) (tree-map-node.key node)) (tree-map-node.value node))) f))) ())))))))
...
(def closures' (fix (fn closures' (fn local-env (fn acc (fn expr ((((symbol-expr? expr) (fn _ ((((null? ((tree-map.get (local-env.locals local-env)) (symbol-expr.name expr))) (fn _ acc)) (fn _ (((tree-map.put acc) (symbol-expr.name expr)) true))) ()))) (fn _ (((fold (list-expr.exprs expr)) acc) (closures' local-env)))) ())))))))
...
(def mpm-resolve-generate-result' (fix (fn mpm-resolve-generate-result' (fn resolved (fn result ((((generate-result.match result) (fn degenerate' (impure ((pair resolved) degenerate')))) ((mpm-resolve-generating mpm-resolve-generate-result') resolved)) ((mpm-resolve-generated mpm-resolve-generate-result') resolved)))))))
...
(def repl (fix (fn repl (fn global-env (fn heap (fn index (fn name (fn resolved ((then-run-with ((then-run ((ostream.write stdout) (car (symbol >)))) (istream.readln stdin))) (fn line (((((symbol.= ((filter line) ((compose not) whitespace?))) ()) (fn _ (impure ()))) (fn _ ((with ((char.= (car line)) (car (symbol !)))) (fn !? ((with ((concat (symbol ""(def "")) ((concat ((append name) space)) ((append (((!? (fn _ (cdr line))) (fn _ line)) ())) close-parentheses)))) (fn text ((with ((repl-parse text) index)) (fn expr ((then-run-with ((mpm-resolve-generate-result' resolved) (((generate-expr expr) default-local-env) global-env))) (fn pair ((((generate-result.match (second pair)) (fn degenerate' ((then-run ((ostream.writeln stdout) (car (degenerate.errors degenerate')))) ((((repl global-env) heap) (nat.inc index)) resolved)))) (fn generating' ((then-run ((ostream.writeln stdout) ((flat-map (generating.dependencies generating')) ((swap append) space)))) ((((repl global-env) heap) (nat.inc index)) resolved)))) (fn generated' ((with ((repl-interpret-declarations generated') heap)) (fn new-heap ((with ((repl-interpret-operation generated') new-heap)) (fn value (((((!? (fn _ then-run-with)) (fn _ with)) ()) value) (fn v (((((((const repl) (debug v)) (generated.global-env generated')) new-heap) (nat.inc index)) ((cons slash) name)) (first pair)))))))))))))))))))) ())))))))))))
(def run-repl (fn args ((((nil? args) (fn _ (((((repl default-global-env) empty-heap) nat.1) (symbol _)) (empty-tree-map compare-symbol)))) (fn _ ((with ((file.child file.local-file) (car args))) (fn file ((then-run-with (parse-file file)) (fn exprs ((then-run-with (generate exprs)) (fn result (((((repl (generated.global-env result)) ((interpret-declarations (generated.declarations result)) empty-heap)) nat.1) (symbol _)) (empty-tree-map compare-symbol)))))))))) ())))

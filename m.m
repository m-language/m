...
(def symbol-expr (fn name (fn path (fn start (fn end ((pair true) ((cons name) ((cons path) ((cons start) ((cons end) nil))))))))))
...
(def list-expr (fn exprs (fn path (fn start (fn end ((pair false) ((cons exprs) ((cons path) ((cons start) ((cons end) nil))))))))))
...
(def fail (fn message (failure ((cons message) nil))))
...
(def assert (fn env (fn exprs (result/success (expr/list ((cons (expr/symbol (symbol delay))) ((cons (expr/list ((cons (expr/symbol (symbol test-condition))) ((cons (car exprs)) ((cons (expr/list ((cons (expr/symbol (symbol symbol))) ((cons (expr/symbol (symbol "Assertion failed"))) nil)))) nil))))) nil)))))))
(def assert-not (fn env (fn exprs (result/success (expr/list ((cons (expr/symbol (symbol assert))) ((cons (expr/list ((cons (expr/symbol (symbol not))) ((cons (car exprs)) nil)))) nil)))))))
...
(def expr.path (fn expr ((((list-expr? expr) (fn _ (list-expr.path expr))) (fn _ (symbol-expr.path expr))) nil)))
(def expr.start (fn expr ((((list-expr? expr) (fn _ (list-expr.start expr))) (fn _ (symbol-expr.start expr))) nil)))
(def expr.end (fn expr ((((list-expr? expr) (fn _ (list-expr.end expr))) (fn _ (symbol-expr.end expr))) nil)))
(def test-condition (fn condition (fn message (((condition (fn _ success)) (fn _ (fail message))) nil))))
...
(def expr.with-path (fn path (fn expr ((((symbol-expr? expr) (fn _ ((((symbol-expr (symbol-expr.name expr)) path) (symbol-expr.start expr)) (symbol-expr.end expr)))) (fn _ ((((list-expr ((map (list-expr.exprs expr)) (expr.with-path path))) path) (list-expr.start expr)) (list-expr.end expr)))) nil))))
...
(def combine-tests (fn tests (fn _ (((fold tests) success) (fn state (fn test (((test nil) (const state)) (fn failure1 ((state (fn _ (failure failure1))) (fn failure2 (failure ((concat failure1) failure2))))))))))))
...
(def do (fn env (fn exprs (result/success ((((nil? (cdr exprs)) (fn _ (expr/list exprs))) (fn _ (expr/list ((cons (expr/symbol (symbol then-run-with))) ((cons (cadr exprs)) ((cons (expr/list ((cons (expr/symbol (symbol fn))) ((cons (car exprs)) ((cons (expr/list ((cons (expr/symbol (symbol do))) (cddr exprs)))) nil))))) nil)))))) nil)))))
...
(def stdout stdout)
(def stderr stderr)
(def run-test (fn test (((test nil) (fn success ((ostream.writeln stdout) (symbol "Tests successful")))) (fn failure (((fold failure) (impure nil)) (fn process (fn message ((then-run process) ((ostream.writeln stderr) message)))))))))
(def stdin stdin)
(def debug debug)
...
(def if-operation ((new-data (symbol if-operation)) ((cons (symbol cond)) ((cons (symbol true)) ((cons (symbol false)) nil)))))
...
(def let (fn env (fn exprs (result/success ((((nil? (cdr exprs)) (fn _ (expr/list exprs))) (fn _ (expr/list ((cons (expr/symbol (symbol with))) ((cons (cadr exprs)) ((cons (expr/list ((cons (expr/symbol (symbol fn))) ((cons (car exprs)) ((cons (expr/list ((cons (expr/symbol (symbol let))) (cddr exprs)))) nil))))) nil)))))) nil)))))
...
(def put-refs (fn declarations (fn ref-root ((with ((filter declarations) def-declaration?)) (fn def-declarations (((fold def-declarations) (impure nil)) (fn process (fn declaration ((then-run process) ((file.write ((file.child ref-root) (normalize (def-declaration.name declaration)))) (def-declaration.path declaration)))))))))))
...
(def interpret-if-operation (fn interpret-operation' (fn operation (fn stack (fn heap ((((((interpret-operation' (if-operation.cond operation)) stack) heap) (fn _ (((interpret-operation' (if-operation.true operation)) stack) heap))) (fn _ (((interpret-operation' (if-operation.false operation)) stack) heap))) nil))))))
...
(def closures' (fix (fn closures' (fn local-env (fn acc (fn expr ((((symbol-expr? expr) (fn _ ((((null? ((tree-map.get (local-env.locals local-env)) (symbol-expr.name expr))) (fn _ acc)) (fn _ (((tree-map.put acc) (symbol-expr.name expr)) true))) nil))) (fn _ (((fold (list-expr.exprs expr)) acc) (closures' local-env)))) nil)))))))

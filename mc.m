...
(def list (fn expr ((((nil? expr) (const ())) (fn _ ((cons (left (symbol cons))) ((cons (car expr)) ((cons (right ((id list) (cdr expr)))) ()))))) ())))
(def & (fn expr ((cons (left (symbol and))) ((cons (car expr)) ((cons (right ((cons (left (symbol delay))) ((cons (cadr expr)) ())))) ())))))
(def | (fn expr ((cons (left (symbol or))) ((cons (car expr)) ((cons (right ((cons (left (symbol delay))) ((cons (cadr expr)) ())))) ())))))
(def if (fn expr ((cons (left (symbol force))) ((cons (right ((cons (car expr)) ((cons (right ((cons (left (symbol delay))) ((cons (cadr expr)) ())))) ((cons (right ((cons (left (symbol delay))) ((cons (caddr expr)) ())))) ()))))) ()))))
(def delay (fn expr ((cons (left (symbol fn))) ((cons (left (symbol _))) expr))))
(def force (fn expr ((cons (car expr)) ((cons (right ())) ()))))
...
(def extern (fn expr ((cons (left (symbol def))) ((cons (car expr)) ((cons (car expr)) ())))))
...
(def apply-vararg (fn expr ((cons (car expr)) ((cons (right ((cons (left (symbol list))) (cdr expr)))) ()))))
(def success (left ()))
(def fail (fn messages (right messages)))
(def run-tests (fn tests (fn _ (((fold tests) success) (fn state (fn test (((test ()) id) (fn fail1 ((state (const fail1)) (fn fail2 (fail ((concat fail1) fail2))))))))))))
(def mc-test ((((run-tests ()) ()) (fn success ((ostream.writeln stdout) (symbol ""Tests successful"")))) (fn fail (((fold fail) (impure ())) (fn process (fn message ((then-run process) ((ostream.writeln stderr) message))))))))

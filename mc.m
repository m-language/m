...
(def parse-file (fn file ((then-run-with (file->tree-map file)) (fn tree-map (((tree-map.fold tree-map) (impure ())) (fn !acc (fn path (fn file ((then-run-with (file.read file)) (fn chars ((then-run-with !acc) (fn acc (impure ((concat acc) (((parse (init (init (cdr ((flat-map path) (cons slash)))))) chars) ((position nat.1) nat.1))))))))))))))))

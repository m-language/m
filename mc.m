...
(def formfeed (car (symbol "")))
(def newline? (fn char ((or ((char.= char) linefeed)) (fn "" ((or ((char.= char) carriage-return)) (fn "" ((char.= char) formfeed)))))))
...
(def escape (fn char (if ((char.= char) letter-b) backspace (if ((char.= char) letter-t) tab (if ((char.= char) letter-n) linefeed (if ((char.= char) letter-v) vtab (if ((char.= char) letter-f) formfeed (if ((char.= char) letter-r) carriage-return char))))))))
...
(def whitespace? (fn char ((or (newline? char)) (fn "" ((or ((char.= char) space)) (fn "" ((or ((char.= char) tab)) (fn "" ((char.= char) vtab)))))))))
...
(def parse (fn input (fn path (fn position (fn acc (if (nil? input) (reverse acc) (if (newline? (car input)) ((((parse (cdr input)) path) (next-line position)) acc) (if (whitespace? (car input)) ((((parse (cdr input)) path) (next-char position)) acc) ((with (((parse-expr input) path) position)) (fn result ((((parse (parse-result.rest result)) path) (expr.end (parse-result.expr result))) ((cons (parse-result.expr result)) acc))))))))))))
